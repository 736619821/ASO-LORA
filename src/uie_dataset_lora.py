...
class UIEInstructions(datasets.GeneratorBasedBuilder):
    ...
    def load_NGUM_dataset(self, dataset_path, labels_path, dataset_name, sampling_strategy, max_num_instances, subset):

        instances, labels = self._load_dataset(dataset_path, labels_path)

        sample_template = {"Task": "NGUM", "Dataset": dataset_name, "Samples": [], "subset": subset}
        ...
        for idx, instance in enumerate(instances):
            example = sample_template.copy()
            instruction = self._get_instruction('NGUM')
            instruction += "Option: " + labels_str + " \n" + "{0}" + "\nAnswer:"
            label = instance['label']
    ...
    def load_MMLU_dataset(self, dataset_path, labels_path, dataset_name, sampling_strategy, max_num_instances, subset):

        instances, labels = self._load_dataset(dataset_path, labels_path)

        sample_template = {"Task": "MMLU", "Dataset": dataset_name, "Samples": [], "subset": subset}
        ...
        for idx, instance in enumerate(instances):
            example = sample_template.copy()
            instruction = self._get_instruction('MMLU')
            instruction += "Option: " + labels_str + " \n" + "{0}" + "\nAnswer:"
            label = instance['label']
    ...
    def _generate_examples(self, path=None, task_config=None, max_num_instances_per_task=None, subset=None):
        ...
        for task in task_config:
            if task == "SC":
                ...
                load_func = self.load_NGUM_dataset
            elif task == 'MMLU':
                load_func = self.load_MMLU_dataset
